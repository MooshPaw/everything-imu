//! Synthetic Steam Deck — feature `synthetic-source`.
...
pub struct SyntheticSteamDeck {
    metadata: DeviceMetadata,
    seed: u64,
    handle: Option<tokio::task::JoinHandle<()>>,
}

impl SyntheticSteamDeck {
    pub fn new(seed: u64) -> Self {
        let mac = derive_mac(seed);
        let id = DeviceId {
            mac,
            serial: format!("synth-deck-{seed:04x}"),
        };
        Self {
            metadata: DeviceMetadata {
                id,
...
impl Device for SyntheticSteamDeck {
    ...
    async fn set_rumble(&mut self, intensity: f32) -> Result<(), DeviceError> {
        tracing::debug!(intensity, "synthetic deck rumble");
        Ok(())
    }
}
...
fn derive_mac(seed: u64) -> [u8; 6] {
    ...
    [0x02, 0x28, 0xDE, bytes[0], bytes[1], bytes[2]]
}
...
mod tests {
    ...
    async fn synthetic_emits_connected_and_samples() {
        let mut dev = SyntheticSteamDeck::new(0xCAFE);
